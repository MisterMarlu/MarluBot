...
class Commands extends CommandList {
  ...
  static lolElo(message, args, bot, debug = false) {
    ...
    Helper.debugging('Variable "location" =>', location, debug);
    Helper.debugging('Variable "name" =>', name, debug);

    LolApi.getElo(name, location, debug).then((info) => {
      let summoner = info.summoner,
        ranked = info.ranked,
        response = `Ranked stats of ${summoner.name} (lvl ${summoner.summonerLevel})`;

      Helper.debugging('Variable "summoner" =>', summoner, debug);
      Helper.debugging('Variable "ranked" =>', ranked, debug);
...
function getCommandList(debug = false) {
  ...
  Helper.debugging('Variable "props" =>', props, debug);

  for (let i = 0; i < props.length; i += 1) {
    let method = props[i];
    Helper.debugging('Variable "method" =>', method, debug);
