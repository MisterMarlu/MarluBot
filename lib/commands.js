...
class Commands extends CommandList {
  static do(message, command, args, bot = null) {
    ...
    if (typeof Commands[method] !== 'function') {
      let response = t('noCommand', command);
  ...
  static lolElo(message, args, bot) {
    ...
    LolApi.getElo(name, location).then((info) => {
      let summoner = info.summoner,
        ranked = info.ranked,
        response = t('eloStart', [summoner.name, summoner.summonerLevel]),
        queue = {
          api: 'RANKED_SOLO_5x5',
          show: 'Solo 5v5',
        };
      ...
      response += t('eloStat', [apiQueue.wins, apiQueue.losses, apiQueue.leaguePoints]);
      message.channel.send(response);
    }).catch(error => {
      message.reply(t('eloError'));
    });
  }
  ...
  static debug(message, command, args, bot) {
    ...
    message.reply(t('debugNoCommand', [command]));
  }
}

function getCommandList() {
  let props = Object.getOwnPropertyNames(Commands),
    response = t('getCommands') + doBreak;
