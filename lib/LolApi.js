...
class LolApi {
   ...
   * @returns {Promise}
   */
  static getCurrentMatchup(name, location) {
    return new Promise((resolve, reject) => {
      LolApi.getSummonerByName(name, location).then(summoner => {
        let url = `${LolApi.buildUrl(location)}/lol/spectator/v3/active-games/by-summoner/${summoner.id}${key}`;
        Helper.debugging('Variable "url" =>', url);
        axios.get(url).then(response => {
          let gameInfo = response.data;

          // console.log(gameInfo.partic

          resolve({summoner, gameInfo});
        }).catch(error => {
          reject('Error on getting the current game info');
          console.log(error);
        });
      });
    });
