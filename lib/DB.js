...
/**
 * This class executes all database operations with a DB database.
 *
...
class DB {
   ...
   * Set connection.
   *
   * @param {connection} connection The connection of the database, set by the DB.
   */
  ...
  insert(insert, table) {
    ...
    let values = DB.parseValues(Object.values(insert)),
  ...
  update(filter, update, table, operator = '=') {
    ...
    // Parse values and create query string.
    let values = DB.parseColumnValue(update, operator),
      whereCondition = DB.parseColumnValue(filter, operator),
  ...
  delete(filter, table, operator = '=') {
    ...
    let whereCondition = DB.parseColumnValue(filter, operator),
  ...
  find(filter, table, structure = {}, operator = '=') {
    // Parse values and create query string.
    let whereCondition = DB.parseColumnValue(filter, operator),
      rows = DB.parseStructure(structure),
  ...
  /**
   * Parse all values to correct notation for the DB query.
   *
   * @param {array} values An array of values.
   * @returns {array} The parsed values as array for the DB query.
   */
  static parseValues(values) {
    let parsedValues = [];

    for (let i = 0; i < values.length; i += 1) {
      parsedValues[i] = DB.parseValue(values[i]);
    }
  ...
  /**
   * Parse the value to correct notation for the DB query.
   *
   * @param {*} value The value to parse.
   * @returns {number|boolean|string} Returns the correct notation for the DB query.
   */
  ...
  static parseColumnValue(object, operator) {
    ...
    for (let column in object) {
      ...
      if (typeof object[column] !== 'object') {
        columnValues.push(`${column} ${operator} ${DB.parseValue(object[column])}`);
        continue;
      }
      ...
      for (let innerColumn in object[column]) {
        ...
        innerVal = DB.parseValue(object[column][innerColumn]);
      }
